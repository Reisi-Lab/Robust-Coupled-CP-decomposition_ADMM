...
% rest target data and same number of standard data to calculate new inputs (lambdas) for SVM.
...
% Set tensor rank for CPD
rank = 16;   % 4 8 16 
num_sample = size(target_allsubs,3);
Label = ones(num_sample,1);
indices = crossvalind('Kfold',Label,K);
size_fold = sum(indices == 1);

lambda_fMRI_target = zeros(num_sample-size_fold, rank);
lambda_fMRI_standard = zeros(num_sample-size_fold, rank);
...
for k = 1:K
    ...
    H = khatrirao(cpdecom.U{2},cpdecom.U{1});
    HH = (H'*H)\H';
  
    for idx = 1:num_sample-size_fold
        target_train = target_allsubs(:,:,train);   
        target_new = target_train(:,:,idx);
        lambda_fMRI_target(idx,:) = (HH * target_new(:))'; 
        
        standard_train = standard_allsubs(:,:,train);
        standard_new = standard_train(:,:,idx);
        lambda_fMRI_standard(idx,:) = (HH * standard_new(:))';        
    end 
    
    lambdas = [lambda_fMRI_target;lambda_fMRI_standard]; 
